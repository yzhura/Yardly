...
export async function GET(_req: Request, { params }: RouteParams) {
  ...
  const res = await fetch(`${getApiBaseUrl()}/tenants/${tenantId}/catalogs`, {
    headers: { Authorization: `Bearer ${token}` },
    cache: "no-store",
  });

  if (!res.ok) {
    return handleApiProxyError(res, "Не вдалося завантажити каталоги");
  }
...
export async function POST(req: Request, { params }: RouteParams) {
  ...
  if (!res.ok) {
    return handleApiProxyError(res, "Не вдалося створити каталог");
  }
