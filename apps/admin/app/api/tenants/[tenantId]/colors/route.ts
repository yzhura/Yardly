...
export async function GET(_: Request, { params }: RouteParams) {
  ...
  const res = await fetch(`${getApiBaseUrl()}/tenants/${tenantId}/colors`, {
    headers: { Authorization: `Bearer ${token}` },
    cache: "no-store",
  });

  if (!res.ok) {
    return handleApiProxyError(res, "Не вдалося завантажити кольори");
  }
...
export async function POST(req: Request, { params }: RouteParams) {
  ...
  if (!res.ok) {
    return handleApiProxyError(res, "Не вдалося створити колір");
  }
